...
\textbf{Grupo} nr. & 112 
\\\hline
a84552 Jose Luis Cerqueira Pires
\\
a80309 Joao Nuno Vieira Carvalho
\\
a82066 Francisco Domingos Martins Oliveira	
\\
a85774 Barbara Paulista de Faria
...
\begin{verbatim}
Tendo em conta que é um isomorfismo temos que : 

inExpAr . outExpAr = id
outExpAr . inExpAr = id 

inExpAr = [ const.X, [ N,[ bin, Ûn] ] ] 
...
expAr a -> Either () (Either a (Either (BinOp, (a, a)) (UnOp, a)))

g_eval_exp x (Left ()) = x
g_eval_exp _ (Right (Left x)) = x
g_eval_exp _ (Right (Right (Left (Sum, (x, y))))) = x + y
g_eval_exp _ (Right (Right (Left (Product, (x, y))))) = x * y
g_eval_exp _ (Right (Right (Right (Negate, x)))) = -x
g_eval_exp _ (Right (Right (Right (E, x)))) = Prelude.exp x 
\end{verbatim}

\begin{code}
g_eval_exp a = either fX (either fN (either fBin fUnOp))
               where fX = const a
                     fN x = x
                     fBin (Sum, (x, y)) = (+) x y
                     fBin (Product, (x, y)) =(*) x  y
                     fUnOp (Negate, x) = negate x
                     fUnOp (E, x) = Prelude.exp x
\end{code}

\begin{code}
clean X = Left ()
clean (N a) = Right $ Left a 
clean (Bin op x y) = Right $ Right $ Left (op , (x,y)) 
clean (Un op x) = Right $ Right $ Right (op , x)
\end{code}

\begin{code}
gopt a = either fX (either fN (either fBin fUnOp))
               where fX = const a
                     fN x = x
                     fBin (Sum, (0, y)) =  y
                     fBin (Sum, (x, 0)) =  x
...
sd_gen (Right (Right (Left (Sum, ((f, f'), (g, g')))))) 
= (Bin Sum f g, Bin Sum f' g')

sd_gen (Right (Right (Left (Product, ((f, f'), (g, g')))))) 
= (Bin Product f g, Bin Sum (Bin Product f g') (Bin Product f' g))

sd_gen (Right (Right (Right (Negate, (f, f'))))) 
= (Un Negate f, Un Negate f')

sd_gen (Right (Right (Right (E, (f, f'))))) 
= (Un E f, Bin Product (Un E f) f')
...
sd_gen  = either fX (either fN (either fBin fUnOp))
               where fX = const ( X , N 1 )
                     fN x = (N x, N 0)
                     fBin (Sum, ((f, f'), (g, g')))  = (Bin Sum f g, Bin Sum f' g')
                     fBin (Product, ((f, f'), (g, g'))) =(Bin Product f g, Bin Sum (Bin Product f g') (Bin Product f' g))
                     fUnOp (Negate, (f, f')) = (Un Negate f, Un Negate f')
                     fUnOp (E, (f, f')) = (Un E f, Bin Product (Un E f) f')
...
ad_gen a (Right (Left x)) = (x, 0) 

ad_gen a (Right (Right (Left (Sum, ((f, f'), (g, g')))))) 
= (f + g, f' + g') 

ad_gen a (Right (Right (Left (Product, ((f, f'), (g, g')))))) 
= (f * g, f * g' + f' * g) 

ad_gen a (Right (Right (Right (Negate, (f, f'))))) 
= (-f, -f') 

ad_gen a (Right (Right (Right (E, (f, f'))))) 
= (Prelude.exp f, f' * Prelude.exp f) 
\end{verbatim}

\begin{code}

ad_gen a = either fX (either fN (either fBin fUnOp))
               where fX = const(a, 1)
                     fN x = (x, 0)
                     fBin (Sum, ((f, f'), (g, g')))  = (f + g, f' + g')
                     fBin (Product, ((f, f'), (g, g'))) =(f * g, f * g' + f' * g) 
                     fUnOp (Negate, (f, f')) =  (-f, -f')
                     fUnOp  (E, (f, f')) = (Prelude.exp f, f' * Prelude.exp f) 
\end{code}
...
Derivar implementação do cálculo do n-ésimo número de Catalan:
...
Seja cat = $c\ n = \frac{(2n)!}{(n+1)!(n!)}$ a função pretendida.
Facilmente se nota que: 
|c 0 = 1|
\begin{eqnarray}
|c (n+1)| 	= \frac{(2(n+1))!}{(n+2)!(n+1)!} \\
			= \frac{(2n+2)!}{(n+2)!(n+1)!} \\
			= \frac{(2n+2)(2n+1)(2n)!}{(n+2)(n+1)n!(n+1)n!} \\
			= \frac{2(n+1)(2n+1)(2n)!}{(n+2)(n+1)n!(n+1)n!} \\
			= 2 * \frac{(2n+1)(2n)!}{(n+2)n!(n+1)n!} \\
			= 2 * \frac{2n+1}{n+2} * \frac{(2n)!}{n!(n+1)n!} \\
			= 2 * \frac{2n+1}{n+2} * \frac{(2n)!}{(n+1)!n!} \\
			= \frac{4n+2}{n+2} * c n 
\end{eqnarray}

Se definirmos $|h n| = 4n+2$ e $|s n| = n+2$, teremos |c| em recurividade mútua com |h| e |s|.

Repetindo o processo para ambas, teremos:
|h 0| = 2
$|h (n+1)| = 4(n+1)+2 = 4n+4+2 = (h n)+4$

|s 0| = 2
$|s (n+1)| = n+1+2 = (s n)+1$


Sendo assim, as funções auxiliares são:

\begin{code}
c :: Integer -> Integer
c 0 = 1
c (n+1) = div((c n) * (h n))(s n)
...
E a solução terá este formato:

\begin{code}

cat = prj . for loop inic where
   loop (c,h,s) = (div(c*h)(s), h+4, s+1)
   inic = (1,2,2)
   prj (c,h,s)= c
...
avg_aux = cataList gene where
          gene = either(b)(q) 
          b  =  const ( 0, 0 )
          q (h, (a, l)) =   ( (/) ( ( (+)  ((*) a  (fromIntegral l) )   h))  ((1+)   (fromIntegral l) ) , (1+)  (fromIntegral l) )
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{cp2021t_media/avgL.png}
\end{figure} 
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{cp2021t_media/avgT.png}
\end{figure}
...
avgLTree = p1.cataLTree gene where
   gene = either(f)(g)
   f x = (x,1)
   g((a1,c1),(a2,c2))= ( ((/) ((+) ((*) a1 c1) ((*) a2 c2)) ((+) c1 c2)), (+) c1 c2  )
